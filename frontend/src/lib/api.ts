...
export async function fetchTrivia(params: FetchTriviaParams): Promise<TriviaQuestion[]> {
  ...
  if (!res.ok) {
    // Token might be expired/exhausted on backend — refresh once
    if (res.status === 400 || res.status === 401 || res.status === 403) {
      const fresh = await fetchNewToken();
      url.searchParams.set("token", fresh);
      const retry = await fetch(url.toString());
      if (!retry.ok) throw new Error("Failed to fetch trivia");
