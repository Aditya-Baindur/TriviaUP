 ...
 * Stats Page
 *
 * Displays performance across all played games.
 * Reads stored game history from localStorage and builds analytics.
 * Shows totals, accuracy, and breakdowns by difficulty and category.
 *
 * Usage :
 *  - Access via "/stats"
 *  - Automatically loads data from localStorage ("triviaup_stats")
 *  - Provides option to clear stored history
 *
 * @note
 *  - Calculates stats client-side (no backend)
 *  - Handles empty state when no games are played
 *  - Limits category breakdown to most frequent entries
 */
...
const Stats = () => {
  ...
  const byDifficulty: Record<Difficulty, { correct: number; total: number }> = {
    ...
    any: { correct: 0, total: 0 }, //Only for type safety, not used
  }
